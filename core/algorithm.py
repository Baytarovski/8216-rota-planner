...
def generate_rota(daily_workers, daily_heads, rotas, inspectors, week_key):
    ...
    # Count weekly presence per inspector
    for day in all_days:
        for worker in daily_workers[day]:
            worker_days[worker] += 1

    # Step 2: Count last 4 weeks history for FCI/OFFLINE
    ...
    # Step 3: Assignment attempts loop
    for _ in range(DEFAULT_ATTEMPTS):
        used = defaultdict(list)  # track assignments per person
        rota_table = {}
        ...
        # Ensure 4+ day workers get at least 1 FCI/OFFLINE
